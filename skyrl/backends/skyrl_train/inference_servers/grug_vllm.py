...
def _align_kv_heads(value: torch.Tensor, num_q_heads: int) -> torch.Tensor:
    ...
    value = value.unsqueeze(2).expand(value.shape[0], num_kv_heads, repeat, value.shape[2])
...
class GrugMoeRuntimeConfig:
    ...
    def from_hf_config(cls, config: Any) -> "GrugMoeRuntimeConfig":
        max_seq_len = int(_config_attr(config, ("max_seq_len", "max_position_embeddings"), 4096))
        ...
        return cls(
            ...
            artifact_schema_version=int(_config_attr(config, ("grugmoe_artifact_schema_version",), 1)),
            ...
            num_experts=int(_config_attr(config, ("num_experts", "num_local_experts"), 8)),
            num_experts_per_token=int(_config_attr(config, ("num_experts_per_token", "num_experts_per_tok"), 2)),
            latent_dim=None if latent_dim is None else int(latent_dim),
            num_layers=num_layers,
            num_heads=num_heads,
            num_kv_heads=int(_config_attr(config, ("num_kv_heads", "num_key_value_heads"), num_heads)),
            local_kv_heads=(None if local_kv_heads is None else int(local_kv_heads)),
            global_kv_heads=(None if global_kv_heads is None else int(global_kv_heads)),
            ...
            layer_norm_eps=float(_config_attr(config, ("layer_norm_eps", "rms_norm_eps"), 1e-5)),
            initializer_std=float(_config_attr(config, ("initializer_std", "initializer_range"), 0.02)),
            qk_mult=float(_config_attr(config, ("qk_mult",), 1.0)),
            qk_mult_long_scale=float(_config_attr(config, ("qk_mult_long_scale",), 1.0)),
            ...
            sconv_sites=tuple(_config_attr(config, ("sconv_sites",), ("k", "attn", "mlp"))),
            tie_word_embeddings=bool(_config_attr(config, ("tie_word_embeddings",), False)),
    ...
    def validate(self) -> "GrugMoeRuntimeConfig":
        if self.artifact_schema_version not in (
            ...
            raise ValueError("unsupported grugmoe_artifact_schema_version=" f"{self.artifact_schema_version}")
        if self.artifact_schema_version == 1:
            ...
            if hero_fields:
                raise ValueError(", ".join(hero_fields) + " require grugmoe_artifact_schema_version=2")
        ...
        if self.num_experts_per_token >= self.num_experts:
            raise ValueError("num_experts_per_token must be < num_experts for QB routing")
        if self.latent_dim is not None and not 0 < self.latent_dim <= self.hidden_dim:
            raise ValueError(f"latent_dim must be in (0, hidden_dim={self.hidden_dim}]")
...
class GrugMoeRouter(BaseRouter):
    ...
    def _compute_routing(
        ...
        *,
        ...
        topk_weights = torch.sigmoid(torch.gather(router_logits.float(), dim=-1, index=selected))
...
class GrugMoeMLP(nn.Module):
    ...
    def __init__(
        ...
        self.latent_norm = RMSNorm(cfg.expert_dim, eps=cfg.layer_norm_eps) if cfg.latent_dim is not None else None
        self.latent_up_proj = (
            ReplicatedLinear(
                cfg.expert_dim,
                cfg.hidden_dim,
    ...
    def _forward_torch_reference(
        ...
        routed_output = torch.sum(expert_out * combine_weights.unsqueeze(-1), dim=1)
    ...
    def forward_impl(
        ...
        if attn_metadata.num_prefills > 0:
            ...
            prefill_output = causal_conv1d_fn(
                ...
                activation=None,
                metadata=(attn_metadata if attn_metadata.nums_dict is not None else None),
...
class GrugMoeAttention(nn.Module):
    ...
    def apply_xsa(
        ...
        value_heads = value_states.view(num_tokens, self.cfg.num_kv_heads, self.head_dim)
...
def _raise_for_unsupported_modes(vllm_config: VllmConfig) -> None:
    ...
    if quant_config is not None and (not is_tpu or quant_config.get_name() != _UNQUANTIZED_METHOD):
        raise NotImplementedError("GrugMoE does not support quantization")
    if not is_tpu and parallel_config.tensor_parallel_size != 1:
        raise NotImplementedError("GrugMoE expert-parallel GPU serving requires tensor_parallel_size=1")
    if not is_tpu and parallel_config.tensor_parallel_size != get_tensor_model_parallel_world_size():
        raise RuntimeError(
...
def _try_load_grug_expert_weight(
    ...
    for weight_name, param_name, shard_id in _EXPERT_WEIGHT_MAPPING:
        ...
        if param is None:
            raise ValueError("Mapped Grug expert weight " f"{name!r} to missing parameter {mapped_name!r}")
        weight_loader = getattr(param, "weight_loader", None)
        if weight_loader is None:
            raise ValueError(f"Grug expert parameter {mapped_name!r} has no FusedMoE weight_loader")
...
class GrugMoeForCausalLM(nn.Module, HasInnerState, IsHybrid, SupportsPP):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            if (mapped_name := _try_load_grug_expert_weight(name, loaded_weight, params_dict)) is not None:
                loaded_params.add(mapped_name)
                continue

            param = params_dict.get(name)
            if param is None:
                raise ValueError(f"Unexpected GrugMoE weight {name!r}; parameter was not found")
