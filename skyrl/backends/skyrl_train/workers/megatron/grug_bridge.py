...
from megatron.bridge.models.conversion import auto_bridge as auto_bridge_module
...
from skyrl.backends.skyrl_train.workers.megatron.grug_model import (
    GrugGPTModel,
    grug_block_spec,
)
from skyrl.models.grug import GRUG_MOE_MODEL_TYPE, GrugMoeConfig, grug_long_layer_flags

GRUG_ROTARY_PERCENT = 0.5

# ModelOpt can replace Transformers' lazy top-level module while Megatron-Bridge
# imports. Use the exact module object held by AutoBridge so architecture lookup
# and bridge registration cannot disagree about the class identity.
transformers = auto_bridge_module.transformers
AutoConfig = transformers.AutoConfig
PreTrainedModel = transformers.PreTrainedModel

# AutoBridge asks Transformers to materialize the HF config before dispatching
# to this registered bridge.
AutoConfig.register(GRUG_MOE_MODEL_TYPE, GrugMoeConfig, exist_ok=True)


class _GrugMoeForCausalLMCompat(PreTrainedModel):
    """Config-only Transformers sentinel used by Megatron-Bridge dispatch.

    Grug weights are streamed directly from safetensors; this class is never
    instantiated for training. Megatron-Bridge 0.4 resolves architecture names
    through the top-level ``transformers`` module before consulting its bridge
    registry, so expose the sentinel there as a compatibility shim.
    """
...
if hasattr(transformers, "GrugMoeForCausalLM"):
    GrugMoeForCausalLM = transformers.GrugMoeForCausalLM
else:
    # Transformers versions predating native Grug support still need an
    # architecture object so Megatron-Bridge 0.4 can resolve config.architectures.
    GrugMoeForCausalLM = _GrugMoeForCausalLMCompat
    transformers.GrugMoeForCausalLM = GrugMoeForCausalLM
...
class GrugMoeBridge(MegatronModelBridge):
    ...
    def provider_bridge(self, hf_pretrained) -> GrugModelProvider:
        ...
        if config.grugmoe_artifact_schema_version != 1:
            raise NotImplementedError(
                "Megatron training currently supports Snowball/Grug artifact schema 1 only; "
                f"got schema {config.grugmoe_artifact_schema_version}"
            )
    ...
    def mapping_registry(self) -> MegatronMappingRegistry:
        mappings = [
            ...
            *_gated_norm_mappings(
            ...
            AutoMapping(
                "decoder.layers.*.self_attention.attn_gate.weight",
