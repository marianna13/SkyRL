...
def init_custom_process_group(
    ...
    # The previous heuristic `str(torch.__version__) >= "2.6"` is a lexicographic
    # string compare and is WRONG for torch >= 2.10 ("2.10" < "2.6" as strings),
    # which picks the old `pg_options` name on a torch that only accepts
    # `backend_options` -> TypeError. Introspect the real signature instead so
    # this is correct for any torch version.
    import inspect as _inspect

    _npg_params = _inspect.signature(_new_process_group_helper).parameters
    if "backend_options" in _npg_params:
        _pg_opt_kw = "backend_options"
    elif "pg_options" in _npg_params:
        _pg_opt_kw = "pg_options"
    else:
        _pg_opt_kw = None

    _npg_kwargs = {"group_name": group_name, "timeout": timeout}
    if _pg_opt_kw is not None:
        _npg_kwargs[_pg_opt_kw] = pg_options
